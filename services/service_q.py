...
class ServiceQ:
    ...
    def wrap_t_read(self, req_id):
        ...
        # Если успели, возвращаем результат
    ...
    def wrap_s_write(self, req_id, data):
        ...
        res = yield write_proc | timeout_event
        if timeout_event in res:
            raise RuntimeError("write timeout")  # Просто "write timeout"
        return True

    def _t_read(self, req_id):
        val = self.service_t.read(
            req_id
        )  # может вызвать RuntimeError если data not found или отказ
        yield self.env.timeout(0)
        return val

    def _t_write(self, req_id, data):
        self.service_t.write(req_id, data)  # может вызвать RuntimeError при отказе
        yield self.env.timeout(0)
        return True

    def _s_read(self, req_id):
        val = yield self.env.process(
            self.service_s.read(req_id)
        )  # может RuntimeError: data not found или отказ
        return val

    def _s_write(self, req_id, data):
        yield self.env.process(
            self.service_s.write(req_id, data)
        )  # может RuntimeError при отказе
        return True
